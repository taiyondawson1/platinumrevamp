        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <MetricCard
              ...
              subValue={`$${metrics.totalProfit >= 0 ? '+' : ''}${metrics.totalProfit.toFixed(2)}`}
            />
            <MetricCard
              label="Closed drawdown 5 days ago"
              value={metrics.maxDrawdown.toFixed(2) + "%"}
              subValue={`$${(metrics.maxDrawdown * (selectedAccount?.balance || 0) / 100).toFixed(2)}`}
            />
            <MetricCard
              label="Float"
              value={`$${metrics.floatingPL.toFixed(2)}`}
              subValue={`${metrics.openOrdersCount} orders`}
            />
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <MetricCard
              label="Average Win"
              value={Math.abs(tradingMetrics.avgWin)}
              subValue={`$${selectedAccount?.balance?.toFixed(2) || '0.00'}`}
            />
            <MetricCard
              label="Average Loss"
              value={Math.abs(tradingMetrics.avgLoss)}
              subValue={`$${selectedAccount?.balance?.toFixed(2) || '0.00'}`}
            />
            <MetricCard
              label="Win Rate"
